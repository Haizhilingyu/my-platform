 ...
 *   2. 清理上次运行残留的 E2E 测试数据（按命名约定 E2E-* / e2e-* 前缀）
 *
...
export default async function globalSetup() {
  ...
  await execute("DELETE FROM sys_user_role WHERE user_id IN (SELECT id FROM sys_user WHERE username LIKE 'e2e-%')")
  await execute("DELETE FROM sys_role_menu WHERE role_id IN (SELECT id FROM sys_role WHERE role_code LIKE 'e2e-%')")
  await execute("DELETE FROM sys_user WHERE username LIKE 'e2e-%'")
  await execute("DELETE FROM sys_role WHERE role_code LIKE 'e2e-%'")
