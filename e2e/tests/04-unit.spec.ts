...
test.describe('单位管理 CRUD + Bug2 回归', () => {
  ...
  test('新增根单位 → 节点恰出现一次（Bug2 回归） + DB 入库', async ({ page }) => {
    ...
    const afterCount = await page.locator('.n-tree-node-content').count()
  ...
  test('新增子单位 → parent_id 正确 + 节点恰出现一次', async ({ page }) => {
    ...
    await page.locator('.n-tree-node-content', { hasText: ROOT_CODE }).first().locator('..').getByRole('button', { name: '新增' }).click()
    await expect(page.getByText('新增单位').first()).toBeVisible()
    ...
    await expect(page.locator('.n-tree-node-content', { hasText: CHILD_CODE })).toBeVisible({ timeout: 10_000 })
  ...
  test('编辑单位 → DB 名称变更', async ({ page, request }) => {
    ...
    await page.locator('.n-tree-node-content', { hasText: ROOT_CODE }).first().locator('..').getByRole('button', { name: '编辑' }).click()
  ...
  test('删除单位 → DB 行删除', async ({ page, request }) => {
    ...
    await page.goto('/sys/unit')
    await page.waitForLoadState('networkidle')
    await page.locator('.n-tree-node-content', { hasText: ROOT_CODE }).first().locator('..').getByRole('button', { name: '删除' }).click()
