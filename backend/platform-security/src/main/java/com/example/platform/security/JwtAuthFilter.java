 ...
 *
 * <p>由 {@link SecurityConfig} 注册到过滤器链，位于 {@code UsernamePasswordAuthenticationFilter} 之前。
 *
 * <p>Token 黑名单校验：解析成功后检查 {@code jwt:blacklist:{jti}} 是否存在。 命中黑名单（已登出）则不注入认证上下文，请求将以 401 拒绝。
 */
...
class JwtAuthFilter extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(
    ...
    if (token != null && jwtUtil.isValid(token)) {
      ...
      boolean blacklisted = jti != null && redisCacheService.exists(BLACKLIST_KEY_PREFIX + jti);
