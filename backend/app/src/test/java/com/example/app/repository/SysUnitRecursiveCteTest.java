...
/**
 * 递归 CTE 单位树解析测试。三级树 总部(100) → 分公司A(101) → 部门1(102) 验证 {@link
 * SysUnitRepository#findDescendantUnitIds(Long)} 在 H2 PostgreSQL 模式下正确返回全部后代（含根）。
 */
@SpringBootTest
@ActiveProfiles("test")
@Transactional
class SysUnitRecursiveCteTest {
  ...
  @Autowired SysUnitRepository unitRepository;
  ...
  void middle_returnsItsSubtree() {
    assertThat(unitRepository.findDescendantUnitIds(101L)).containsExactlyInAnyOrder(101L, 102L);
  }
