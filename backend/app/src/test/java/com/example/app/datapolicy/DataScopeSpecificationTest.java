...
/**
 * 数据权限 Specification 五种范围的端到端校验：在 H2（PostgreSQL 模式）真实库上插入带 {@code unit_id}
 * 与 {@code created_by} 的样本行，断言每种 {@link DataScope} 生成的 Predicate 过滤结果正确。
 *
 * <p>使用 @SpringBootTest + test profile（复用 ApplicationContextLoadsTest 已验证的 H2+Flyway 上下文），
 * @Transactional 保证每个用例的数据改动在用例结束时回滚。
 */
@SpringBootTest
@ActiveProfiles("test")
@Transactional
class DataScopeSpecificationTest {
    ...
    void seed() {
        jdbc.update(
                "INSERT INTO sys_user (id, username, password, unit_id, status, created_by)"
                        + " VALUES (1001, 'u_hq', 'x', 1, 1, '" + ME + "')");
        jdbc.update(
                "INSERT INTO sys_user (id, username, password, unit_id, status, created_by)"
                        + " VALUES (1002, 'u_unit2', 'x', 2, 1, '" + OTHER + "')");
        jdbc.update(
                "INSERT INTO sys_user (id, username, password, unit_id, status, created_by)"
                        + " VALUES (1003, 'u_self_other_unit', 'x', 1, 1, '" + OTHER + "')");
    }
    ...
    void all_returnsEverything() {
        DataScopeContext.ScopeData ctx = new DataScopeContext.ScopeData(DataScope.ALL, null, null, null);
    ...
    void unitBelow_returnsAllDescendantUnitRows() {
        ...
        assertThat(idsWith(ctx, Set.of(1L, 2L)))
                .containsExactlyInAnyOrder(1L, 1001L, 1002L, 1003L);
    }
