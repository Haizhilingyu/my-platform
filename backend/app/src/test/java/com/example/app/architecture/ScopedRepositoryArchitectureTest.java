...
/**
 * ArchUnit 架构约束：处理 {@link ScopedEntity}（带数据权限范围）的 Repository 接口必须继承
 * {@link ScopedRepository}，从而获得 {@code JpaSpecificationExecutor} 与 {@code scopeFindById} 能力。
 */
class ScopedRepositoryArchitectureTest {
    ...
    void repositoriesOverScopedEntitiesMustExtendScopedRepository() {
        ...
        List<Class<?>> violating = classes.stream()
        ...
        assertThat(violating)
                .as("处理 ScopedEntity 的 Repository 必须继承 ScopedRepository")
                .isEmpty();
    }
