 ...
 * 平台架构边界守护（ArchUnit）。
 *
 * <p>本测试集与 {@link ScopedRepositoryArchitectureTest}（数据权限 Repository 约束）和
 * platform-common 的 {@code ArchUnitTest}（common 不反向依赖业务）互补，覆盖：
 *
 * <ol>
 *   <li><b>模块边界</b>：平台基础层（common / platform.security）不得反向依赖业务模块；
 *       核心业务模块（sys / audit / openapp）之间不得相互依赖。
 *   <li><b>命名规范</b>：controller / service / repository 包内类名必须以对应后缀结尾。
 *   <li><b>分层方向</b>：Controller 不得直接访问 Repository（必须通过 Service）。
 *   <li><b>分层架构</b>：Controller → Service → Repository 单向依赖（consideringOnlyDependenciesInLayers
 *       忽略对 common / DTO / framework 的依赖，避免误报）。
 * </ol>
 *
 * <p>已知合法跨模块依赖（不视为违规）：
 * <ul>
...
class ArchitectureBoundaryTest {
    ...
    /**
     * platform-common 是最底层基础模块，禁止依赖任何业务模块（sys / audit / notify / openapp /
     * loginldap），否则将破坏分层架构。
     *
     * <p>与 platform-common 自带的 {@code ArchUnitTest} 语义一致，但此处扫描全量 classpath 做一次集中校验，
     * 覆盖所有业务模块包名。
     */
    @Test
    ...
    /**
     * platform-security 提供 JWT 过滤器与 SecurityConfig，只依赖 platform-common 的抽象（CurrentUser /
     * JwtUtil / PermissionLoader），不得直接耦合 sys 业务实现。
     *
     * <p>这是 T2 解耦的核心成果：SecurityConfig 通过 {@code PermissionLoader} 接口（在 common 定义）调用
     * sys 的 PermissionService，编译期无 sys 依赖。
     */
    @Test
    void platformSecurityShouldNotDependOnSys() {
        noClasses()
                ...
                .resideInAnyPackage(
                        ...
                        "com.example.loginldap..")
                .because(
                        "platform-security 只依赖 common 抽象（PermissionLoader/JwtUtil），"
                                + "不得直接耦合业务模块实现")
                .check(classes);
    }
    ...
    /**
     * audit 是审计日志写入与查询模块，只通过 {@code @Auditable} 注解 + common 的 AuditAspect 解耦采集，
     * 不得直接依赖任何业务模块。
     */
    @Test
     ...
     *
     * <p>允许 Controller 依赖 common（Result / CurrentUser / 注解）、DTO、domain（返回类型）与 framework；
     * 但任何对 {@code ..repository..} 包的引用都视为越层。
     *
     ...
     *
     * <p>使用 {@code consideringOnlyDependenciesInLayers()} 只检查层间依赖，忽略对 common / DTO /
     * framework / 第三方的依赖，避免误报。Repository 不得依赖 Controller 或 Service；Service 不得依赖
     * Controller。
     */
    @Test
