...
/**
 * 权限加载 SPI。由 platform-common 定义，业务模块（如 sys）提供实现， 上层（app/SecurityConfig）只依赖本接口，从而解耦对具体业务模块的编译期依赖。
 *
 * <p>这是应用层安全（JWT 过滤器、权限填充）所需的最小契约：
 *
 * <ul>
 *   <li>{@link #loadPermissions(Long)} / {@link #loadRoles(Long)}：加载标识集合
 *   <li>{@link #loadUserInfo(Long)}：组装 {@link CurrentUser.UserInfo} 记录
 *   <li>{@link #hasPermission(Long, String)}：单权限校验
 * </ul>
 *
