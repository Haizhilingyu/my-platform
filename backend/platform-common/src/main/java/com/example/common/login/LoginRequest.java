 ...
 * 同理 {@code captchaId}/{@code captchaCode} 缺失时为 null，由 AuthController 按配置决定是否放行。
 *
 ...
 * @param attributes  扩展属性映射，承载非通用字段：LDAP 域、SSO token 等。
...
public record LoginRequest(
     ...
     *
     * <p>保留此构造器是为了让历史调用点（如 {@code new LoginRequest(method, user, pw, attrs)}）
     * 无需改动；record 的规范构造器已扩展为 6 参。
     */
    public LoginRequest(String method, String username, String password, Map<String, Object> attributes) {
