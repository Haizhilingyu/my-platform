 ...
 *
 * <p>组合 {@link JpaRepository} 与 {@link JpaSpecificationExecutor}，并提供按数据权限范围过滤的 {@link
 * #scopeFindById(Serializable)} 默认方法。实体类型上界为 {@link ScopedEntity}， 保证 {@link
 * DataScopeSpecification} 能统一引用 {@code unitId} / {@code createdBy} 字段。
 *
 * @param <T> 范围实体类型
 * @param <ID> 主键类型
 */
public interface ScopedRepository<T extends ScopedEntity, ID extends Serializable>
  ...
  /**
   * 按主键查询并应用当前线程的数据权限过滤。{@code UNIT_BELOW} 范围需调用方预先解析后代单位 ID， 传入 {@link
   * #scopeFindById(Serializable, Set)} 重载；本重载以空集降级（等价于仅本单位）。
   *
   * @param id 主键
