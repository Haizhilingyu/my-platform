 ...
 * 图形验证码服务。
 *
 * <p>生成（{@link #generate()}）使用 Hutool {@link LineCaptcha}，答案存 Redis（key= {@code
 * captcha:{captchaId}}，TTL 5 分钟）。
 *
 * <p>校验（{@link #validate(String, String)}）命中后立即删除 Redis key，确保单次使用—— 同一 captchaId 第二次校验必然失败，防止重放攻击。
 */
@Service
