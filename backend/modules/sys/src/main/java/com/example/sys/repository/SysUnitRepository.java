...
public interface SysUnitRepository extends JpaRepository<SysUnit, Long> {
     ...
     *
     * <p>使用标准 SQL {@code WITH RECURSIVE}（非 PG 专有语法），H2 2.x PostgreSQL 模式与
     * PostgreSQL 12+ 均支持。注意：CTE 必须显式声明列名 {@code unit_tree(id, parent_id)}，否则
     * H2 在递归成员的 JOIN ON 中无法解析 {@code unit_tree.id}（PG 不受此限制但兼容显式列名）。
     * 用于 {@link com.example.common.datapolicy.DataScope#UNIT_BELOW} 范围解析。
     *
     * @param rootId 根单位 ID
