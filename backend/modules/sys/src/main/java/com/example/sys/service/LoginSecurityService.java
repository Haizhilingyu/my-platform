 ...
 *
 * <p>失败计数 key: {@code login:fail:{username}}（首次失败设 30min TTL）。 锁定 key: {@code
 * user:lock:{username}} = "1"（TTL 同失败窗口）。{@code incr} 为 Redis INCR 原子操作，并发安全。
 *
...
public class LoginSecurityService {
    ...
    public int getFailCount(String username) {
        return redisCacheService
                .get(failKey(username), Long.class)
                .map(Long::intValue)
                .orElse(0);
    }
    ...
    private int resolveThreshold() {
        try {
            String value = configService.getValue(MAX_FAIL_CONFIG_KEY, String.valueOf(DEFAULT_MAX_FAIL_COUNT));
