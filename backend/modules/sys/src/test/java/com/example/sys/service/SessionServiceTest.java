...
class SessionServiceTest {
    ...
    private LoginSuccessEvent event(String userAgent) {
        return new LoginSuccessEvent(1L, "admin", "jti-123", "10.0.0.1", userAgent, LocalDateTime.now());
    }
    ...
    class RecordSession {
        ...
        void recordSession_storesSessionAndIndex() {
            ...
            verify(redisCacheService).set(eq(SessionService.SESSION_KEY_PREFIX + "jti-123"),
                    captor.capture(), eq(TTL));
    ...
    class ListSessions {
        ...
        void listSessions_returnsActiveAndCleansStale() {
            ...
            SessionInfo info1 = new SessionInfo("jti-1", 1L, "admin", "10.0.0.1",
    ...
    class RevokeSession {
        ...
        void revokeSession_blacklistsAndDeletes() {
            when(redisTemplate.opsForSet()).thenReturn(setOps);
            SessionInfo info = new SessionInfo("jti-1", 1L, "admin", "10.0.0.1",
                    "Chrome", "Chrome", LocalDateTime.now(), LocalDateTime.now().plusHours(1));
            when(redisCacheService.get(SessionService.SESSION_KEY_PREFIX + "jti-1", SessionInfo.class))
                    .thenReturn(Optional.of(info));
            when(redisTemplate.getExpire(SessionService.SESSION_KEY_PREFIX + "jti-1"))
                    .thenReturn(3500L);

            service.revokeSession("jti-1");

            verify(redisCacheService).set(eq(SessionService.BLACKLIST_KEY_PREFIX + "jti-1"),
        ...
        void revokeSession_zeroTtl_skipsBlacklist() {
            when(redisTemplate.opsForSet()).thenReturn(setOps);
            SessionInfo info = new SessionInfo("jti-2", 2L, "user", "10.0.0.2",
                    "FF", "Firefox", LocalDateTime.now(), LocalDateTime.now());
            when(redisCacheService.get(SessionService.SESSION_KEY_PREFIX + "jti-2", SessionInfo.class))
                    .thenReturn(Optional.of(info));
            when(redisTemplate.getExpire(SessionService.SESSION_KEY_PREFIX + "jti-2"))
                    .thenReturn(0L);
    ...
    class DeviceTypeParsing {
        ...
        @Test
        void chrome() {
            assertThat(SessionService.parseDeviceType(
        ...
        @Test
        void firefox() {
            assertThat(SessionService.parseDeviceType("Mozilla/5.0 Firefox/121.0"))
                    .isEqualTo("Firefox");
        }
        ...
        @Test
        void edge() {
            assertThat(SessionService.parseDeviceType("Mozilla/5.0 Edg/120.0.0.0"))
                    .isEqualTo("Edge");
        }
        ...
        @Test
        void postman() {
            assertThat(SessionService.parseDeviceType("PostmanRuntime/7.36.0"))
                    .isEqualTo("Postman");
        }
