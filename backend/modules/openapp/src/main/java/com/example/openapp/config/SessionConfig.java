 ...
 *
 * <p>这是 OIDC RP-Initiated Logout 实现高可用（HA）的基础。授权服务器（AS）在多副本部署时，任一副本收到 {@code /oauth2/logout}
 * 请求并销毁会话后，该销毁操作作用于 Redis 中的共享会话——所有副本立即感知，避免出现「A 副本登出但 B 副本仍持有有效会话」的单点残留。
 *
 ...
 *   <li>测试环境（{@code application-test.yml}）Redis 指向 localhost:6379 但无实例运行，NO_OP 跳过连接尝试， 保证
 *       {@code @SpringBootTest} 上下文正常加载（符合「gracefully handle test profile」要求）。
 * </ol>
 *
 * <p>如需会话过期事件（如 T11 在线会话的 TTL 过期感知），运维方在 Redis 中手动执行 {@code CONFIG SET notify-keyspace-events Elg}
 * 即可，Spring Session 会自动订阅。
 */
@Configuration
