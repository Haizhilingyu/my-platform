 ...
 * <ul>
 *   <li>enabled 未设置（默认 false）→ 不注册任何 LDAP bean（provider/contextSource/ldapTemplate）。</li>
 *   <li>用 ApplicationContextRunner（轻量，无需启动完整应用上下文）。</li>
 * </ul>
 *
 * <p>enabled=true 的实际装配由 app 模块的 ApplicationContextLoadsTest（profile=test，
 * ldap 默认关闭）间接覆盖；provider 业务正确性由 {@link LdapLoginProviderTest} 覆盖。
 */
...
class LdapLoginAutoConfigurationTest {
    ...
    void whenDisabled_noLdapBeansRegistered() {
        runner.withConfiguration(AutoConfigurations.of(LdapLoginAutoConfiguration.class))
                .run(
                        context -> {
