 ...
 * }</pre>
 *
 * <p>{@code user-dn-pattern} 中的 {@code {0}} 占位符由用户名替换，语义对齐 Spring Security
 * LDAP 的 {@code userDnPatterns}。{@code enabled} 默认 false，确保未配置环境不激活 LDAP。
 */
@Getter
@Setter
...
public class LdapLoginProperties {
    ...
    /**
     * 用户 DN 模板，{@code {0}} 为用户名占位符。
     * 例：{@code uid={0},dc=devenv,dc=local} → {@code uid=hai,dc=devenv,dc=local}。
     */
