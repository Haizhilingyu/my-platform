 ...
 * LDAP 登录自动配置。
 *
 * <p>仅当 {@code platform.login.ldap.enabled=true} 时激活（{@link ConditionalOnProperty}）。
 * 激活时注册：
 * <ul>
 *   <li>{@link ContextSource}（{@link LdapContextSource}）：无 base、无 manager 凭证，
 *       仅用于用户 bind（{@code getContext(userDn, password)}）。</li>
 *   <li>{@link LdapTemplate}：基于上述 ContextSource，供后续扩展（高阶 LDAP 操作）。</li>
 *   <li>{@link LdapLoginProvider}：登录方式提供者，自动收集到 LoginMethodRegistry。</li>
 * </ul>
 *
