 ...
 *
 * <ol>
 *   <li>用 {@code user-dn-pattern} 拼出用户完整 DN（{@code {0}} 替换为用户名）。
 *   <li>通过 {@link ContextSource#getContext(String, String)} 执行 LDAP bind。 成功即认证通过，{@link
 *       AuthenticationException} 即凭据错误（→ 401）。
 *   <li>从认证上下文读取 {@code mail}/{@code cn} 属性，作为自动建号信息。
 *   <li>查本地 SysUser：存在则校验状态；不存在且 {@code auto-create-user=true} 则自动建号 并分配 {@code default-role-code}
 *       角色。
 *   <li>加载角色、生成 JWT（与 PasswordLoginProvider 同构）、发布 LoginSuccessEvent、 返回 LoginVO（结构同密码登录）。
 * </ol>
 *
 * <p>本类只做读认证 + 本地用户 provision，不向 LDAP 写数据。仅当 {@code platform.login.ldap.enabled=true} 时由 {@link
 * LdapLoginAutoConfiguration} 注册。
 */
@Slf4j
  ...
  private SysUser createLocalUser(LdapUserInfo ldapUser) {
    SysUser user =
        SysUser.builder()
            ...
            .realName(ldapUser.realName() != null ? ldapUser.realName() : ldapUser.username())
            .email(ldapUser.email())
            .status(1)
            .build();
    user = userRepository.save(user);
    assignDefaultRole(user.getId());
    log.info("LDAP 首次登录自动创建本地用户: id={}, username={}", user.getId(), user.getUsername());
    return user;
  }
