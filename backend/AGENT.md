...
- **提交前本地全绿**：至少跑变更模块 `mvn test`。红测试禁止提交；`--no-verify` 仅限不碰测试范围的 WIP。
- **修改既有行为时先改测试**：测试即规格说明，行为变更必须先体现为断言变更。
- **测试暴露实现 bug 时，先保留 failing test 再修实现**（TDD 的核心价值时刻）。
- 覆盖率硬门禁：JaCoCo ≥80% 行（CI `jacoco:check` 红灯即阻塞）。
